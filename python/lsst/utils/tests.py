...
try:
    ...
    # ignore leaked blocks with IDs before memId0
    memId0 = 0
    # maximum number of leaked blocks to print
    nleakPrintMax = 20
...
def sort_tests(tests):
    """!Go through the supplied sequence of test suites and sort them to ensure
    that MemoryTestCases are at the end of the test list. Returns a combined
    TestSuite."""
...
class MemoryTestCase(unittest.TestCase):
    ...
    def testFileDescriptorLeaks(self):
        ...
        if diff:
            for f in diff:
                print("File open: %s" % f)
            self.fail("Failed to close %d file%s" %
                      (len(diff), "s" if len(diff) != 1 else ""))
...
class ExecutablesTestCase(unittest.TestCase):
    ...
    def assertExecutable(self, executable, root_dir=None, args=None, msg=None):
        ...
        try:
            ...
            output = e.output
            failmsg = "Bad exit status from '{}': {}".format(
                executable, e.returncode)
    ...
    def _build_test_method(cls, executable, root_dir):
        ...
        @param root_dir Path to executable. Not used if executable path is
        absolute.
        """
...
def findFileFromRoot(ifile):
    ...
    we start in $cwd and walk up until we find the file (or throw IOError if it
    doesn't exist)

    This is useful for running tests that may be run from _dir_/tests or
    _dir_"""
...
def getTempFilePath(ext):
    """!Return a path suitable for a temporary file and try to delete the file
    on success

    If the with block completes successfully then the file is deleted, if
    possible; failure results in a printed warning.
    If the block exits with an exception the file if left on disk so it can be
    examined.

    @param[in] ext  file name extension, e.g. ".fits"
    @return path for a temporary file. The path is a combination of the
    caller's file path and the name of the top-level function, as per this
    simple example:
    @code
    ...
    class FooTestCase(unittest.TestCase):
        ...
        def runTest(self):
            with lsst.utils.tests.getTempFilePath(".fits") as tmpFile:
                ...
                # deleted, but only if the file exists and the "with" block
                # terminated normally (rather than with an exception)
    ...
    @endcode
    """
    stack = inspect.stack()
...
def assertRaisesLsstCpp(testcase, excClass, callableObj, *args, **kwargs):
    warnings.warn("assertRaisesLsstCpp is deprecated;"
                  " please just use TestCase.assertRaises",
                  DeprecationWarning)
...
def debugger(*exceptions):
    ...
    e.g., "@debugger(RuntimeError, NotImplementedError)".
    This defaults to just 'AssertionError', for use
...
def plotImageDiff(lhs, rhs, bad=None, diff=None, plotFileName=None):
    ...
    @param[in]  bad            A 2-d boolean NumPy array of values to emphasize
                               in the plots
    @param[in]  diff           difference array; a 2-d NumPy array, or None to
                               show lhs-rhs
    @param[in]  plotFileName   Filename to save the plot to.  If None, the plot
    ...
    for n, (image, title) in enumerate([(lhs, "lhs"), (rhs, "rhs"),
                                        (diff, "diff")]):
        pyplot.subplot(2, 3, n + 1)
        im1 = pyplot.imshow(image, cmap=pyplot.cm.gray,
                            interpolation='nearest',
                            origin='lower', vmin=vmin1, vmax=vmax1)
        if bad is not None:
            pyplot.imshow(badImage, alpha=0.2,
                          interpolation='nearest', origin='lower')
        pyplot.axis("off")
        pyplot.title(title)
        pyplot.subplot(2, 3, n + 4)
        im2 = pyplot.imshow(image, cmap=pyplot.cm.gray,
                            interpolation='nearest',
                            origin='lower', vmin=vmin2, vmax=vmax2)
        if bad is not None:
            pyplot.imshow(badImage, alpha=0.2,
                          interpolation='nearest', origin='lower')
        pyplot.axis("off")
        pyplot.title(title)
    pyplot.subplots_adjust(left=0.05, bottom=0.05, top=0.92,
                           right=0.75, wspace=0.05, hspace=0.05)
...
def assertFloatsAlmostEqual(testCase, lhs, rhs, rtol=sys.float_info.epsilon,
    ...
    The test assertion will fail if all elements lhs and rhs are not equal to
    within the tolerances specified by rtol and atol.  More precisely,
    the comparison is:

    abs(lhs - rhs) <= relTo*rtol OR abs(lhs - rhs) <= atol

    If rtol or atol is None, that term in the comparison is not performed at
    all.

    When not specified, relTo is the elementwise maximum of the absolute values
    of lhs and rhs.  If set manually, it should usually be set to either lhs or
    rhs, or a scalar value typical of what is expected.

    @param[in]  testCase       unittest.TestCase instance the test is part of
    @param[in]  lhs            LHS value(s) to compare; may be a scalar or
                               array-like of any dimension
    @param[in]  rhs            RHS value(s) to compare; may be a scalar or
                               array-like of any dimension
    @param[in]  rtol           Relative tolerance for comparison; defaults to
                               double-precision epsilon.
    @param[in]  atol           Absolute tolerance for comparison; defaults to
                               double-precision epsilon.
    @param[in]  relTo          Value to which comparison with rtol is relative.
    @param[in]  printFailures  Upon failure, print all inequal elements as part
                               of the message.
    @param[in]  plotOnFailure  Upon failure, plot the originals and their
                               residual with matplotlib.
                               Only 2-d arrays are supported.
    @param[in]  plotFileName   Filename to save the plot to.  If None, the plot
                               will be displayed in a window.
    @param[in]  invert         If True, invert the comparison and fail only if
                               any elements *are* equal. Used to implement
                               assertFloatsNotEqual, which should generally be
                               used instead for clarity.
    @param[in] msg             String to append to the error message when
                               assert fails.
    """
        ...
        if numpy.isscalar(bad):
            if rtol is None:
                ...
                errMsg = ["%s %s %s; diff=%s/%s=%s with rtol=%s"
                          % (lhs, cmpStr, rhs, absDiff, relTo, absDiff/relTo,
                             rtol)]
            else:
            ...
            if plotOnFailure:
                ...
                try:
                    plotImageDiff(lhs, rhs, bad, diff=diff,
                                  plotFileName=plotFileName)
                except ImportError:
                    errMsg.append("Failure plot requested but matplotlib"
            ...
            if printFailures:
                ...
                if rtol is None:
                    for a, b, diff in zip(lhs[bad], rhs[bad], absDiff[bad]):
                        errMsg.append("%s %s %s (diff=%s)" %
                                      (a, cmpStr, b, diff))
                else:
                    for a, b, diff, rel in zip(lhs[bad], rhs[bad],
                                               absDiff[bad], relTo[bad]):
                        errMsg.append("%s %s %s (diff=%s/%s=%s)" %
                                      (a, cmpStr, b, diff, rel, diff/rel))
...
def assertFloatsEqual(testCase, lhs, rhs, **kwargs):
    ...
    """
    return assertFloatsAlmostEqual(testCase, lhs, rhs, rtol=0, atol=0,
                                   **kwargs)


@inTestCase
def assertClose(*args, **kwargs):
    warnings.warn("assertClose is deprecated; please use"
                  " TestCase.assertFloatsAlmostEqual",
                  DeprecationWarning)
...
def assertNotClose(*args, **kwargs):
    warnings.warn("assertNotClose is deprecated; please use"
                  " TestCase.assertFloatsNotEqual",
                  DeprecationWarning)
